 ...
 * 
 * @author toulain.timothé@u-cergy.fr
 **/
public class CheckerBoard extends JPanel implements CustomEvent {
	...
	private DrawGUI draw = new DrawGUI();
	static JFrame applicationFrame;
	private static CheckerBoard instance=new CheckerBoard("checker",applicationFrame);

	public CheckerBoard(String title,JFrame frame) {
		super();
		//init(frame);
		
	}
	private static CheckerBoard getInstance() {
		return instance;
	}
    ...
    public void stateChanged(JFrame frame) {
        ...
        this.init(frame);
        System.out.println("Fenetre de jeu créée");
    }
	...
	private void creerCase() {
		// TODO - Essayer de créer une collection qui contient des données comme les couleurs de chaque point et leur position ( éventuellement leur futur propriétaire ), pour ensuite appeler une fonction qui parcourt cette collection et créer chaque point
